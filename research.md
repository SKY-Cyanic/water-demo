...
### 개발 서버가 죽었는지부터 본다 (Phase 19 추가)

`qa.mjs`가 "app did not finish booting within 90s"를 세 번 냈고 세 번 다
개발 서버가 죽어 있었다. 부팅 실패는 셰이더 컴파일 문제처럼 보이기 때문에
매번 코드부터 뒤졌다.

**`curl -s -o /dev/null -w "%{http_code}" http://localhost:8173/` 를 먼저 친다.**
1초면 되고, "부팅 실패"를 봤을 때 그것이 코드에 관한 정보인지 아닌지를 가른다.
장시간 QA 루프 중에 서버가 죽는 원인은 아직 모른다 — 알아내기 전까지는
이 확인이 대책이다.

### 같은 규칙에 두 번 걸렸다 — 기준선 재측정은 *습관*이어야 한다 (Phase 19 추가)

SSR을 넣고 플리커가 3.17 % → 4.12 %로 올라 회귀로 판단했다. 원인을 하드 히트
판정으로 추측해 부드럽게 바꿨다 — 변화 없음(0.423 대 0.421). 그제서야 동일 세션에서
**SSR 이전 빌드를 다시 쟀더니 0.433 %**였다. 회귀는 없었다.

Phase 16에 정확히 같은 실수를 하고 바로 아래에 규칙으로 적어두고도 반복했다.
이유는 분명하다: 그때는 "기준선을 의심하라"를 *교훈*으로 적었지 *절차*로 만들지
않았다. 교훈은 잊히고 절차는 남는다.

**절차:** 회귀로 보이는 수치를 만나면, 원인을 생각하기 전에 먼저
`git stash && git checkout <ref> -- src/`로 이전 빌드를 돌려 같은 세션에서 다시 잰다.
2분이면 된다. 추측 한 번이 그보다 비싸다.

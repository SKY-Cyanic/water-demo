...
- **갑판 설비** — 조타륜(림은 스트럿 링, 스포크는 허브로), 시트 윈치 4, 앞갑판 해치,
  양묘기, 콕핏 코밍. 실루엣은 하나도 바꾸지 않는다 — 전부 근접 시점용이다.
  빈 갑판은 선체가 아무리 좋아도 "배의 모형"으로 읽힌다.
- **리그** — 케치. 메인 + 미즌 + 지브에 개프, 스테이·슈라우드, 스탠션과 라이프라인 난간.
  전부 합쳐 3,608 삼각형이다. 멀리서 슬루프는 삼각형 하나로 읽히는데, 뒤쪽의 낮은 마스트
  하나가 그걸 깬다 — 실루엣에 쓸 수 있는 가장 싼 큰 변화다.
